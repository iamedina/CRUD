...
# Esta es la conxeión a la base de datos
...
def consulta_general():
    ...
    try:
        ...
        data = []
        for row in datos:
            dato = {'id_baul': row[0], 'Plataforma': row[1], 'usuario': row[2], 'contrasena': row[3]}
            data.append(dato)
        cur.close()
        conn.close()
        ...
        print(ex)
        return jsonify({'mensaje': 'Error al consultar el baul de contraseñas'})
