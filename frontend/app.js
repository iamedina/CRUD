...
function visualizar(data) {
    ...
    tabla.innerHTML = data.baul.map(item => {
        ...
        return `<tr data-id="${item.id_baul}"><td class="record-id">${item.id_baul}</td><td><span class="platform-name"><i class="bi bi-globe2"></i>${item.Plataforma}</span></td><td>${item.usuario}</td><td><span class="password-value">${clave}</span></td><td class="actions"><a class="icon-button edit" href="edit.html?variable1=${item.id_baul}" aria-label="Editar ${item.Plataforma}" title="Editar"><i class="bi bi-pencil-square"></i></a><button type="button" class="icon-button delete" onclick="eliminar(${item.id_baul})" aria-label="Eliminar ${item.Plataforma}" title="Eliminar"><i class="bi bi-trash3"></i></button></td></tr>`;
    }).join('');
}
